...
users <- orders %>%
  ...
  summarise(
    ...
    #
    use_days_max = max(days_since_prior_order),
    use_days_min = min(days_since_prior_order)
    #
  )
...
#user_average_basket, user_reorder_ratio,time_since_last_order,user_reorder_distinct_products_ratio,days_compare_avg
#-threshold

subtrain <- trainn %>% sample_frac(0.5)
#s_thre<-apply(subtrain%>% select(threshold, up_first_order),1,function(x) toString(x[1]))
int_thre <- apply(subtrain%>% select(threshold, up_first_order),1,function(x) x[1]>0.19)


X <- xgb.DMatrix(as.matrix(subtrain %>% select(user_average_basket, user_reorder_ratio,time_since_last_order,
                                               user_reorder_distinct_products_ratio,days_compare_avg)), label = int_thre)
...
test$threshold <- predict(model_threshold, X)

test$reordered <- (test$reordered > test$threshold/100+0.19 ) * 1
...
write.csv(submission, file = "~/Kaggle/Instacart/submit_multi", row.names = F)
